...
@dataclass
class OT2Model:
    """Encapsulates all OT2-specific state and configuration."""

    # Joint mapping (joint index -> joint name)
    JOINT_NAMES = [
    ...
    # Home positions
    HOME_POSITIONS: Dict[str, float] = field(default_factory=lambda: {
        "pipette_rail_joint": 0.0,
        "pipette_casing_joint": 0.0,
        "pipette_left_joint": 0.0,
        "pipette_right_joint": 0.0,
        "left_plunger_joint": 0.0,
        "right_plunger_joint": 0.0,
        "left_tip_ejector_joint": 0.0,
        "right_tip_ejector_joint": 0.0,
    ...
    # Instance state
    virtual_joints: Dict[str, float] = field(default_factory=lambda: {
        "left_plunger_joint": 0.0,
        "right_plunger_joint": 0.0,
        "left_tip_ejector_joint": 0.0,
        "right_tip_ejector_joint": 0.0,
    })
    left_tip_attached: bool = False
    right_tip_attached: bool = False
    button_light: bool = False
    rail_lights: bool = False
    ...
    def get_joint_limits(self, joint_name: str) -> Tuple[float, float]:
        """Get joint limits for the specified joint."""
        return self.JOINT_LIMITS.get(joint_name, (0.0, 0.0))

    def get_home_position(self, joint_name: str) -> float:
        """Get home position for the specified joint."""
        return self.HOME_POSITIONS.get(joint_name, 0.0)
...
class ZMQ_OT2_Server(ZMQ_Robot_Server):
    ...
    def __init__(self, simulation_app, robot, robot_prim_path, robot_name: str, port: int):
        ...
        # Use OT2Model for configuration and state
        self.model = OT2Model()

        # Aliases for backward compatibility
        self.joint_names = OT2Model.JOINT_NAMES
        self.joint_limits = self.model.JOINT_LIMITS
        self.home_positions = self.model.HOME_POSITIONS
        self.virtual_joints = self.model.virtual_joints
    ...
    def is_virtual_joint(self, joint_name: str) -> bool:
        """Check if a joint is virtual (simulated but not physically controlled)"""
        return self.model.is_virtual_joint(joint_name)
    ...
    def get_joint_positions(self):
        ...
        try:
            ...
            return {"status": "success", "message": "joints retrieved", "joint_positions": joint_positions}
        except Exception as e:
        ...
        try:
            ...
            instruments = {
                "left": {
                    ...
                    "tip_attached": self.model.left_tip_attached,
                },
                "right": {
                    "model": "p300_single_v2.1",
                    "tip_attached": self.model.right_tip_attached,
                }
            }
    ...
    def set_button_light(self, state):
        ...
        try:
            self.model.button_light = bool(state)
            print(f"Button light set to: {self.model.button_light}")
            return {
                "status": "success",
                "button_light": self.model.button_light
            }
        except Exception as e:
    ...
    def set_rail_lights(self, state):
        ...
        try:
            self.model.rail_lights = bool(state)
            print(f"Rail lights set to: {self.model.rail_lights}")
            return {
                "status": "success",
                "rail_lights": self.model.rail_lights
            }
        except Exception as e:
    ...
    def get_lights(self):
        ...
        try:
            lights = {
                "button": self.model.button_light,
                "rails": self.model.rail_lights
            }
