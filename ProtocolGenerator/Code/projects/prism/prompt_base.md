...
Using the provided reference material, create all OT-2 python files and workflow yaml files for the following workflow.
Group all adjacent liquid transfer operations into a single execution of a liquid handling protocol on the OT-2 robot. Come up with a unique and descriptive name for each OT-2 protocol Python file.
All steps outside of liquid handling, including any in comments or described as manual, should be handled in the yaml file.

## Instructions

1. Write the OT-2 protocol Python file(s) to `projects/prism/`. Each file should follow the OT-2 format specification above. Use descriptive filenames (e.g., `pcr_master_mix_and_sample_prep.py`).

2. Write the workflow YAML file to `projects/prism/`. Use a descriptive filename ending in `.yaml` (e.g., `pcr_amplification.yaml`). The YAML must follow the **YAML Workflow File Format Specification** above (WEI `flowdef` format). Do NOT use any other format.

3. Run `bash projects/prism/run.sh` from the ProtocolGenerator/Code/ directory to test the workflow in the simulation. The script automatically converts the WEI YAML to MADSci format and runs the full system (Isaac Sim + REST Gateway + MADSci + Workflow).

4. Check the run summary output. If the workflow fails:
   - Read the log files listed in the summary to identify the failure.
   - Common failure modes:
     - **Plate orientation mismatch**: The plate must be in the correct orientation (wide or narrow) for each instrument. Use the exchange deck to rotate between wide and narrow.
     - **Lid not opened**: The thermocycler and Hidex must be opened before placing a plate, and closed before running a program/assay.
     - **Plate not detected**: The plate must be physically present at the instrument location for operations like seal, peel, run_program, and run_assay.
     - **Collision detected**: The PF400 collided with an instrument during transfer. Check that the correct approach paths are used.
   - Fix the YAML and/or Python files and run again.

5. Iterate until the run summary shows all steps as `ActionStatus.SUCCEEDED` and the workflow log ends with "PCR simulation workflow completed successfully!".
