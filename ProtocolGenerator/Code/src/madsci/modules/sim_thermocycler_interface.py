...
class SimThermocycler:
    ...
    def __init__(
        ...
        self.logger.log(f"SimThermocycler connected to ZMQ server at {self.zmq_server_url}")
    ...
    def open(self) -> bool:
        ...
        self.logger.log("Opening thermocycler lid")
        ...
        if success:
            self.logger.log("Thermocycler lid opened")
        else:
            self.logger.log(f"Failed to open lid: {response.get('message', 'Unknown error')}")

        return success

    def close(self) -> bool:
        """Close thermocycler lid."""
        self.logger.log("Closing thermocycler lid")
        ...
        success = response.get("status") == "success"
        if success:
            self.logger.log("Thermocycler lid closed")
        else:
            self.logger.log(f"Failed to close lid: {response.get('message', 'Unknown error')}")
