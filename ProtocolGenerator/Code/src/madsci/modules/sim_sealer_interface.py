...
class SimSealer:
    ...
    def __init__(
        ...
        self.logger.log(f"SimSealer connected to ZMQ server at {self.zmq_server_url}")
    ...
    def seal(self) -> bool:
        """Seal a plate."""
        self.logger.log("Executing seal operation")

        zmq_command = {"action": "seal"}
        response = self.send_zmq_command(zmq_command)

        success = response.get("status") == "success"
        if success:
            self.logger.log("Seal operation completed")
        else:
            self.logger.log(f"Seal operation failed: {response.get('message', 'Unknown error')}")
